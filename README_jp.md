...
コントローラーポートのVcc（pin1、pin6）からの給電で単独動作が可能です。GND（pin9）はArduinoと共通にしてください。
注意: サターン本体とUSBを同時に接続しないでください。スケッチの書き込みはSaturnから切り離した状態でUSBケーブルを使って行ってください。動作が不安定な場合はpin1/pin6を浮かせてUSBから給電してください。
